...
function enableButtons () {
	...
	btn1= document.getElementById("input_greet_by_period");
	btn1.value="Click me for a personal greeting with period";
	btn1.onclick=function(){greetByPeriod();};
	
	btn2= document.getElementById("input_greet_by_fullname");
	btn2.value="Click me for a full greeting";
	btn2.onclick=function(){greetByFullName();};
}
...
function greetByFullName(){
	var name = document.getElementById("name_field").value;
	var surname = document.getElementById("full_field").value;
	var period = document.getElementById("period_field").value; 
	var request = gapi.client.helloworldendpoints.greetByFullName({'name': name,'surname': surname, 'period':period});
	request.execute(sayHelloCallback);
	}
